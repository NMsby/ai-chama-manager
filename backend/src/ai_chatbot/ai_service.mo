...
module {
  ...
  public func generateSuggestions(userMessage: Text, topics: [Text]) : [Text] {
    ...
    if (Text.size(formatted) > 500) {
        // Simple truncation using substring approach
        let maxLength = 450;
        if (Text.size(formatted) <= maxLength) {
        formatted
        } else {
        // Get first 450 characters using Text module functions
        let iterator = Text.toIter(formatted);
        var result = "";
        var count = 0;
        
        label truncate_loop for (char in iterator) {
            if (count >= maxLength) {
            break truncate_loop;
            };
